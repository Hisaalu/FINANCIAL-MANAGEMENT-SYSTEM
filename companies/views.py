...
def redirect_to_default(request):
    # Your logic to determine the default redirect URL goes here
    default_redirect_url = reverse('companies', args=[1, 1])
    print(default_redirect_url)  # Add this line for testing
...
@custom_login_required
def update_company(request, data, company_uuid):
    # Get Company instance.
    try:
        company = Company.objects.get(unique_id=company_uuid)
    except ValidationError:
