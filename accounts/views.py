...
def custom_logout(request, data):
    ...
    # Adding custom data to be used in the template
    data['alert'] = ('success', 'Logout successfully!', 'You have successfully logged out!')
    data['redirect_link'] = reverse('login')
    
    # Render your custom template with the added context
    return redirect(data['redirect_link'])
...
def change_password(request, data):
    ...
    return render(request, 'accounts/change_password.html', {'form': form, 'data': data})
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.staff.email = form.cleaned_data.get('email')
            user.save()
